...
const ChatbotUI = () => {
  ...
  const fetchChatGPTResponse = async (message) => {
    ...
    if (response.ok) {
      ...
      setMessages((msgs) => [
        ...msgs,
        { id: msgs.length, text: data.answer, sender: "bot" },
      ]);
    } else {
      // Handle error
  ...
  const sendMessage = (message) => {
    setMessages([
      ...messages,
      { id: messages.length, text: message, sender: "user" },
    ]);
          ...
          >
            <div
              ...
              className={`px-4 py-2 mb-2 ${
                message.sender === "user"
                  ? "max-w-[85%] text-right bg-emerald-200 justify-items-end rounded-xl"
                  : "max-w-[85%] text-left bg-emerald-400 basis-4/5 place-content-start rounded-xl"
              }`}
            >
              {message.text}
            </div>
